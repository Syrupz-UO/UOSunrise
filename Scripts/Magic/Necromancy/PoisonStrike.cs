		...
		public void Target( Mobile m )
		{
			if( CheckHSequence( m ) )
			{
				...
				int sdiBonusPVM = AosAttributes.GetValue( Caster, AosAttribute.SpellDamage );
					if ( Server.Misc.DifficultyLevel.SpellDamageIncreaseVsMonsters() > 0 && sdiBonusPVM > Server.Misc.DifficultyLevel.SpellDamageIncreaseVsMonsters() )
					{
						sdiBonusPVM = Server.Misc.DifficultyLevel.SpellDamageIncreaseVsMonsters();
					}

				int sdiBonusPVP = AosAttributes.GetValue( Caster, AosAttribute.SpellDamage );
				if ( Server.Misc.DifficultyLevel.SpellDamageIncreaseVsPlayers() > 0 && sdiBonusPVP > Server.Misc.DifficultyLevel.SpellDamageIncreaseVsPlayers() )
				{
					sdiBonusPVP = Server.Misc.DifficultyLevel.SpellDamageIncreaseVsPlayers();
				}
