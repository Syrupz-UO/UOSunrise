		...
		public static void MyHelpAdmin_OnCommand( CommandEventArgs e )
		{
			...
			if ( ! from.HasGump( typeof( SpeechGump ) ) )
			{
					string sText = ""
						...
						+ "There is a DifficultyLevel.cs file that allows you to change some parameters of how difficult the game is. The gold monsters and containers drop is cut down to 25%, but can be changed in this file. Also, each dungeon has a difficulty level set here. Some dungeons are harder than others, but the loot is better. Be wary when changing these settings as these have been tested to work well for solo players, which is what this server was designed around.<br><br>"

						+ "<br><br>"

						+ "The game will save if you log out in an inn, tavern, home, etc. If you plan to run this in a multi-player fashion, you may want to disable this feature. To do this, find the DifficultyLevel.cs file and change the SaveOnCharacterLogout to false. Either way, the game saves itself every 30 minutes.<br><br>"
