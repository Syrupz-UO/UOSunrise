...
namespace Server.Misc
{
    class MorphingTime
    {
		...
		public static void MakeSpaceAceMetalArmorWeapon( Item item, string material )
		{
			...
			else if ( item is MagicJewelryRing || item.ItemID == 0x108A || item.ItemID == 0x1F09 ){ item.Name = material + suffix + " Ring"; }
			else if ( item.ItemID == 0x1085 || item.ItemID == 0x1F05 || item.ItemID == 0x1F0A ){ item.Name = material + suffix + " Beads"; }
			else if ( item is MagicJewelryNecklace || item.ItemID == 0x1088 || item.ItemID == 0x1F08 ){ item.Name = material + suffix + " Necklace"; }
			else if ( item is MagicJewelryEarrings || item.ItemID == 0x1087 || item.ItemID == 0x1F07 ){ item.Name = material + suffix + " Earrings"; }
			else if ( item is MagicJewelryBracelet || item.ItemID == 0x1086 || item.ItemID == 0x1F06 ){ item.Name = material + suffix + " Bracelet"; }	
		...
		public static void MakeSpaceAceBoneArmor( Item item, string material, bool keepName )
		{
			...
			else if ( item is BoneLegs ){ item.Name = "skeletal leggings"; }
			else if ( item is BoneGloves ){ item.Name = "skeletal gauntlets"; }
			else if ( item is BoneArms ){ item.Name = "skeletal bracers"; }
			else if ( item is BoneChest ){ item.Name = "skeletal tunic"; }
			else if ( item is BoneSkirt ){ item.Name = "skeletal skirt"; }
			else if ( item is BoneHelm ){ item.Name = "skeletal helm"; item.ItemID = 0x1451; }
		...
		public static void MakeSpaceAceItem( Item item, Mobile m )
		{
			if ( m != null )
			{
				...
				if ( Server.Misc.Worlds.IsOnSpaceship( m.Location, m.Map ) )
				{
					...
					else if ( ( item is ClothCowl || item is ClothHood || item is Robe ) && Utility.RandomBool() )
					{
		...
		{
			if ( from is EpicCharacter || from is Citizen )
				return;
		...
		public static bool CheckNecromancer( Mobile from )
		{
			...
			if ( ( from.Region.IsPartOf( "the Undercity of Umbra" ) || from.Region.IsPartOf( "the Black Magic Guild" ) ) && ( from.Hue != 0x83E8 ) )
			{
