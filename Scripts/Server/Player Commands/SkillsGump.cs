...
namespace Server.Gumps 
{
    public class SkillTitleGump : Gump
    {
        public SkillTitleGump ( Mobile from ) : base ( 25,25 )
        {
			...
			AddHtml( 175, 86, 500, 21, @"<BODY><BASEFONT Color=White><BIG>CHOOSE YOUR SKILL TITLE</BIG></BASEFONT></BODY>", (bool)false, (bool)false);
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 99 ){ DB.CharacterSkill = 0; from.PlaySound( 0x4A ); }
			else if ( info.ButtonID > 0 ){ DB.CharacterSkill = info.ButtonID; from.PlaySound( 0x4A ); }
