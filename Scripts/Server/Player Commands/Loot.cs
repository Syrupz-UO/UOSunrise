...
namespace Server.Gumps 
{
    public class LootChoices : Gump
    {
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID < 1 && m_Origin > 0 ){ from.PlaySound( 0x4A ); from.SendGump( new Server.Engines.Help.HelpGump( from, 12 ) ); }
			else if ( info.ButtonID < 1 ){ }
			else { from.SendGump( new LootChoices( from, m_Origin ) ); from.PlaySound( 0x4A ); }
		}
    }
}