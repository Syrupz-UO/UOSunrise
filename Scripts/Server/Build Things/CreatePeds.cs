...
namespace Server.Misc
{
    class BuildSteadPeds
    {
		...
		public static void CreateStealPeds()
		{
			...
			foreach ( Item item in World.Items.Values )
			if ( ( item is FlamesBase ) || ( item is FlamesBaseEmpty ) || ( item is BaneBase ) || ( item is BaneBaseEmpty ) || ( item is PaganBase ) || ( item is PaganBaseEmpty ) )
			{
				FMtargets.Add( item );
			}
		...
		public static void CreateSpecialPedestal( int choice, int ped, string category )
		{
			...
			switch ( choice )
			{
				...
				case 17 : specialPed.MoveToWorld (new Point3D(3882, 3281, 40), Map.Trammel); break; // the Catacomb
