	...
	public class AutoSave : Timer
	{
		...
		protected override void OnTick()
		{
			...
			else
			{
				...
				if ( m > 0 && s > 0 )
					World.Broadcast( 0x35, true, "The shard will save in {0} minute{1} and {2} second{3}.", m, m != 1 ? "s" : "", s, s != 1 ? "s" : "" );
				else if ( m > 0 )
					World.Broadcast( 0x35, true, "The shard will save in {0} minute{1}.", m, m != 1 ? "s" : "" );
				else
					World.Broadcast( 0x35, true, "The shard will save in {0} second{1}.", s, s != 1 ? "s" : "" );
