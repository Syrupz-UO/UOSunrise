    ...
    class LoggingFunctions
    {
		...
		public static string LogEvent( string sEvent, string sLog )
		{
			if ( LoggingFunctions.LoggingEvents() == true )
			{
				...
				string sPath = "/var/www/html/Info/adventures.txt";

				if ( sLog == "Logging Adventures" ){ sPath = "/var/www/html/Info/adventures.txt"; }
				else if ( sLog == "Logging Quests" ){ sPath = "/var/www/html/Info/quests.txt"; }
				else if ( sLog == "Logging Battles" ){ sPath = "/var/www/html/Info/battles.txt"; }
				else if ( sLog == "Logging Deaths" ){ sPath = "/var/www/html/Info/deaths.txt"; }
				else if ( sLog == "Logging Murderers" ){ sPath = "/var/www/html/Info/murderers.txt"; }
				else if ( sLog == "Logging Journies" ){ sPath = "/var/www/html/Info/journies.txt"; }
				else if ( sLog == "Logging Server" ){ sPath = "/var/www/html/Info/server.txt"; }
		...
		public static string LogRead( string sLog, Mobile m )
		{
			...
			if ( nBlank == 1 )
			{
				...
				else if ( sLog == "Logging Adventures" ){ sLogEntries = sLogEntries + "Sorry, " + m.Name + ". I have no new tales of adventure to tell."; }
		...
		{
			if ( !Directory.Exists( "/var/www/html/Info/Articles" ) )
				Directory.CreateDirectory( "/var/www/html/Info/Articles" );
			...
			string path = "/var/www/html/Info/Articles/" + text + ".txt";
		...
		public static string LogSpeak()
		{
			...
			if ( sLog == "Logging Battles" ){ sPath = "/var/www/html/Info/battles.txt"; }
			else if ( sLog == "Logging Deaths" ){ sPath = "/var/www/html/Info/deaths.txt"; }
			else if ( sLog == "Logging Journies" ){ sPath = "/var/www/html/Info/journies.txt"; }
		...
		public static string LogSpeakQuest()
		{
			...
			string sPath = "/var/www/html/Info/quests.txt";
		...
		public static string LogRegions( Mobile m, string sRegion, string sDirection )
		{
			...
			{
				int nDifficulty = DifficultyLevel.GetDifficultyLevel( m.Location, m.Map );
				string sDifficulty = "";
			...
			if ( ( m is PlayerMobile ) && ( m.AccessLevel < AccessLevel.GameMaster ) )
			{
				...
				if (pm.PublicMyRunUO == true)
				{
					...
					if ( sDirection == "enter" ){ sEvent = m.Name + " " + sTitle + " entered " + sRegion + " " ; LoggingFunctions.LogEvent( sEvent, "Logging Journies" ); }
					// else { sEvent = m.Name + " " + sTitle + " left " + sRegion + "#" ; LoggingFunctions.LogEvent( sEvent, "Logging Journies" ); }
				}
			}
			return null;
		}
		...
		public static string LogBattles( Mobile m, Mobile mob )
		{
			...
			if ( m is PlayerMobile && mob != null )
			{
				...
				if ( pm.PublicMyRunUO == true )
				{
					...
					string sEvent = m.Name + " " + sTitle + " had slain " + Killed + " " ;
				...
				else
				{
					...
					string sEvent = m.Name + " " + sTitle + " had slain " + privateEnemy + " " ;
					LoggingFunctions.LogEvent( sEvent, "Logging Battles" );
		...
		public static string LogTraps( Mobile m, string sTrap )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sTrip + " " + sTrap + " " ;
		...
		public static string LogVoid( Mobile m, string sTrap )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sTrap + ", teleporting them far away" ;
				LoggingFunctions.LogEvent( sEvent, "Logging Adventures" );
		...
		public static string LogKillTile( Mobile m, string sTrap )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " made a fatal mistake from " + sTrap + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Adventures" );
		...
		public static string LogLoot( Mobile m, string sBox )
		{
			string sDateString = GetPlayerInfo.GetTodaysDate();
			string sTitle = "the " + GetPlayerInfo.GetSkillTitle( m );
			if ( m.Title != null ){ sTitle = m.Title; }
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + " " + sBox + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Adventures" );
		...
		public static string LogSlayingLord( Mobile m, string creature )
		{
			if ( m != null )
			{
				...
				if ( m is PlayerMobile )
				{
					...
					if (pm.PublicMyRunUO == true)
					{
						string sEvent = m.Name + " " + sTitle + " " + verb + " " + creature + " " ;
		...
		public static string LogCreatedArtifact( Mobile m, string sArty )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = "The gods have created a legendary artifact called " + sArty + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogGenericQuest( Mobile m, string sText )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sText + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogQuestBody( Mobile m, string sBox )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + " " + sBone + " of " + sBox + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogQuestChest( Mobile m, string sBox )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + " " + sChest + " chest of " + sBox + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogQuestMap( Mobile m, int sLevel, string chest )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + " " + chest + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogQuestSea( Mobile m, int sLevel, string sShip )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + " " + sChest + " from " + sShip + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogQuestKill( Mobile m, string sBox, Mobile t )
		{
			...
			if (pm.PublicMyRunUO == true)
			{
				string sEvent = m.Name + " " + sTitle + " " + sLoot + sWho + " " ;
				LoggingFunctions.LogEvent( sEvent, "Logging Quests" );
		...
		public static string LogAccess( Mobile m, string sAccess )
		{
            ...
            if ( m.AccessLevel < AccessLevel.GameMaster )
            {
				...
				if ( sAccess == "login" )
				{
					sEvent = m.Name + " " + sTitle + " had entered the realm" ;
					World.Broadcast(0x35, true, "{0} {1} has entered the realm", m.Name, sTitle);
				
				}
				else
				{
					sEvent = m.Name + " " + sTitle + " had left the realm" ;
			...
			if ( m is PlayerMobile && mob != null )
			{
				...
				if ( m.AccessLevel < AccessLevel.GameMaster )
				{
					...
					if ( pm.PublicMyRunUO == true )
					{
						if ( ( mob == m ) && ( mob != null ) )
						{
							sEvent = m.Name + " " + sTitle + " had killed themselves" ;
						...
						else if ( ( mob != null ) && ( mob is PlayerMobile ) )
						{
							...
							sEvent = m.Name + " " + sTitle + " had been killed by " + sKiller + kTitle + " " ;
						...
						{
							sEvent = m.Name + " " + sTitle + " had been killed" ;
					...
					else
					{
						...
						{
							sEvent = m.Name + " " + sTitle + " had been killed by " + privateEnemy + " " ;
