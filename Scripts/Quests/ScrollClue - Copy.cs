	...
	public class ScrollClue : Item
	{
		...
		public ScrollClue( ) : base( 0x2258 )
		{
			...
			if ( ScrollLevel > 0 ){} else
			{
				...
				ScrollTrue = 1; if ( Utility.RandomMinMax( 1, 2 ) == 1 ){ ScrollTrue = 1; }
				if(ScrollTrue==0)
				{
					ScrollTrue = 1;
				}	
		...
		public override void OnDoubleClick( Mobile e )
		{
			if(Scroll_Map==null)
			{
				e.SendMessage("You realize this is a false map and destroy it while cursing at Mr. T.");
				this.Delete();
				return;
			}
			...
			else
			{
				...
				////////////////////////
						ScrollcluenoobGate gate = new ScrollcluenoobGate();
                        Map map = e.Map;
                        gate.TargetMap = this.Scroll_Map;

                        //ts.Decoder = e; // old tmap sys

                        int z;
                        z = gate.TargetMap.GetAverageZ( this.Scroll_X, this.Scroll_Y );
                        Point3D p = new Point3D(this.Scroll_X, this.Scroll_Y, z );
                        
                        gate.Target = new Point3D(p);
                        gate.MoveToWorld(new Point3D(e.Location),e.Map);

                        
				///////////////////////
				if ( ( e.Int >= ScrollIntelligence && ScrollIntelligence > 0 ) || e.CheckSkill( SkillName.Inscribe, 0, 120 ) )
				{
		  ...
		  //////////////////////
		   public class ScrollcluenoobGate : Moongate
            {
                  
                  [Constructable] 
                  public ScrollcluenoobGate() : base()
                  { 
                  } 
                  ...
                  public override void OnGateUsed(Mobile m)
		      {
                  ...
                  public ScrollcluenoobGate( Serial serial ) : base( serial ) 
                  { 
                  } 
