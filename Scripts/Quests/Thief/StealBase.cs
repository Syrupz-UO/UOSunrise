...
namespace Server.Items
{
	public class StealBase : BaseAddon
	{
		...
		public void FillMeUp( Container box, Mobile from )
		{
			...
			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				box.DropItem( DungeonLoot.RandomSlayer() );
			}

			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				box.DropItem( Loot.RandomSArty() );
			}
			...
			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				...
				{
					box.DropItem( DungeonLoot.RandomEvil() );
				}
				else
				{
			...
			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				box.DropItem( DungeonLoot.RandomLoreBooks() );
			}

			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				if ( Utility.Random( 4 ) != 1 ) { box.DropItem( Loot.RandomScroll( 0, 7, SpellbookType.Regular ) ); }
				else { box.DropItem( Loot.RandomScroll( 0, 17, SpellbookType.Necromancer ) ); }
			}

			box.DropItem( new Gold( ( from.Luck + Utility.RandomMinMax( 1000, 4000 ) ) ) );

			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				Item item = Loot.RandomArmorOrShieldOrWeaponOrJewelry( Server.LootPackEntry.IsInTokuno( from ) );
			...
			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				box.DropItem( Loot.RandomPotion() );
			}

			if ( Server.Misc.GetPlayerInfo.LuckyPlayer( (int)( 20 + ( from.Luck / 2 ) ) ) == true )
			{
				Item item = Loot.RandomWand(); Server.Misc.MaterialInfo.ColorMetal( item, 0 ); string wandOwner = ""; if ( Utility.RandomMinMax( 1, 3 ) == 1 ){ wandOwner = Server.LootPackEntry.MagicWandOwner() + " "; } item.Name = wandOwner + item.Name; box.DropItem( item );
			}
		}
	}
}