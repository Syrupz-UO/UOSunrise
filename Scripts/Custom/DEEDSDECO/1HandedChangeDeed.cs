...
namespace Server.Items
{
    public class LayerTarget : Target // Create our targeting class (which we derive from the base target class)
    {
		...
		protected override void OnTarget( Mobile from, object target ) // Override the protected OnTarget() for our feature
		{
			...
			if ( target is BaseWeapon )
			{
				...
				{
                    from.SendMessage("You Cant change that to a 1 handed weapon!!!"); 
				
				}
				else
				{
                    baseweapon.Layer = Layer.OneHanded;
                    from.SendMessage("You changed it to a 1 handed weapon!!!"); 
	...
	public class OneHandedChangeDeed : Item // Create the item class which is derived from the base item class
	{
		...
		{
            get { return "1 Handed Change Deed"; }
		}

		[Constructable]
		public OneHandedChangeDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
		...
		public override void OnDoubleClick( Mobile from ) // Override double click of the deed to call our target
		{
			...
			{
                from.SendMessage("What would you like to change to a 1 handed weapon?");
