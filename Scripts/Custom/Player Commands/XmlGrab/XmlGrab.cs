		...
		private static void GetItems(LootData lootoptions, Mobile from, ArrayList loothopefuls)
		{
			...
			foreach(Item item in loothopefuls)
			{
				...
				if(itemstoloot != null)
				{
					...
					{
						if ( !DocLootBag.TryDropItem( from, itemx, false ) )
							DocLootBag.DropItem( itemx );
...
// ================================  make sure we have a loot bag
		...
		{
			Container DocLootBag = from.Backpack.FindItemByType( typeof(DocLootBag) ) as Container;
			return ( null == DocLootBag ) ? from.Backpack : DocLootBag;
		}

// ================================  reward the player for not leaving those stinking corpses lying about
		public static void AwardGold( Mobile from, Corpse corpse, Container DocLootBag )
		{
			...
			if ( !DocLootBag.TryDropItem( from, new Gold(amount), false ) )	// Attempt to stack it
				DocLootBag.DropItem( new Gold(amount) );
			from.PlaySound( 0x2E6 ); // drop gold sound
