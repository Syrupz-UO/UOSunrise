...
{
	public class PlainDiamond : Item
	{
		private string m_ModifierList;
	
		[Constructable]
		public PlainDiamond() : base( 7847 )
		{
			Stackable = true;
			Weight = 1;
			Name = "a plain diamond";
			Hue = 2301;
		}
		...
		{ 
			private PlainDiamond m_Augmentation; 
		
			public InternalTarget( PlainDiamond augmentation ) : base( 1, false, TargetFlags.None ) 
			{ 
					...
					else if ( Weapon.Type == WeaponType.Ranged )
					{
						...
						else
						{
							...
							if ( EnhanceChance == 0 ) // Success
							{
								...
								Weapon.WeaponAttributes.HitPhysicalArea += 20;
								Weapon.Hue = 2301;			                  
					...
					|| Armor.BodyPosition == ArmorBodyType.Chest ) 
					{
						...
						{  
							...
							else
							{
								...
								{
									Armor.UsedSockets += 1;
									Armor.Attributes.AttackChance += 4;
									Armor.Hue = 2301;			                  
					...
					else if ( Shield.MaterialType == ArmorMaterialType.Plate ) 
					{
						...
						else
						{
							...
							if ( EnhanceChance == 0 ) // Success
							{
								...
								Shield.EnergyBonus += 3;
								Shield.FireBonus += 3;
								Shield.PoisonBonus += 3;
								Shield.PhysicalBonus += 3;
								Shield.ColdBonus += 3;
								Shield.Hue = 2301;			                  
		...
		public PlainDiamond( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1050045," \tW +20% PhysArea - A +4% Hit - S +3% AllRes\t " );
		}
