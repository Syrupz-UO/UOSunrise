...
{
	public class ChippedDiamond : Item
	{
		private string m_ModifierList;
	
		[Constructable]
		public ChippedDiamond() : base( 7847 )
		{
			Stackable = true;
			Weight = 1;
			Name = "a chipped diamond";
			Hue = 2401;
		}
		...
		{ 
			private ChippedDiamond m_Augmentation; 
		
			public InternalTarget( ChippedDiamond augmentation ) : base( 1, false, TargetFlags.None ) 
			{ 
					...
					|| Armor.BodyPosition == ArmorBodyType.Chest ) 
					{
						...
						{  
							...
							else
							{
								...
								{
									Armor.UsedSockets += 1;
									Armor.Attributes.AttackChance += 2;
									Armor.Hue = 2401;			                  
					...
					else if ( Shield.MaterialType == ArmorMaterialType.Plate ) 
					{
						...
						else
						{
							...
							if ( EnhanceChance == 0 ) // Success
							{
								...
								Shield.EnergyBonus += 1;
								Shield.FireBonus += 1;
								Shield.PoisonBonus += 1;
								Shield.PhysicalBonus += 1;
								Shield.ColdBonus += 1;
								Shield.Hue = 2401;			                  
		...
		public ChippedDiamond( Serial serial ) : base( serial )
		{
		}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			list.Add( 1050045," \tW +10% PhysArea - A +2% Hit - S +1% AllRes\t " );
		}
