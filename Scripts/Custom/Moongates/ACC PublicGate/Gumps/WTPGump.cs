...
{
	public class WTPGump : Gump
	{
		private Mobile     m_From;
		private int        m_Page;
		private WorldTeleporter m_Gate;
		...
		public WTPGump( Mobile from, int Page, WorldTeleporter worldTP ) : base( 0, 0 )
		{
			if( !PGSystem.Running )
				return;
            ...
            AddHtml(0, CStart + 15, 230, 20, "<BASEFONT COLOR=#58D3F7 SIZE=8><CENTER>Personal Teleporter</CENTER></BASEFONT>", false, false);
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if( m_From.Deleted || m_From.Map == null )
				return;
			...
			if( BID == 1 )
			{
				if( Loc <= -1 )
				{
					...
					from.SendGump( new WTPGump( from, m_Page, m_Gate ) );
					return;
				}
				...
				{
					from.SendLocalizedMessage( 1019002 ); // You are too far away to use the worldTP.
				}
			...
			{
				from.CloseGump( typeof( WTPGump ) );
				from.SendGump( new WTPGump( from, BID-100, m_Gate ) );
			}
			...
			else if( BID == 2 && from.AccessLevel >= PGSystem.PGAccessLevel)
			{
				...
				from.CloseGump( typeof( WTPAddEditGump ) );
				from.CloseGump( typeof( WTPGump ) );
				from.SendGump( new WTPGump( from, m_Page, m_Gate ) );
				from.SendGump( new WTPAddEditGump( Flags, m_Page, -1, m_Gate ) );
			}
            ...
            else if (BID == 4 && from.AccessLevel >= PGSystem.PGAccessLevel)
			{
				...
				from.CloseGump( typeof( WTPGump ) );
				from.SendGump( new WTPGump( from, m_Page, m_Gate ) );
				from.SendGump( new WTPAddEditGump( Flags, m_Page, Loc, m_Gate ) );
			}
            ...
            else if (BID == 6 && from.AccessLevel >= PGSystem.PGAccessLevel)
			{
				...
				{
					from.SendMessage( "You must place the worldTP in the World to add it." );
					from.SendGump( new WTPGump( from, m_Page, m_Gate ) );
					return;
				}
                ...
                from.SendGump(new WTPGump(from, m_Page, m_Gate));
                from.SendGump(new WTPAddEditGump(Flags, m_Page, PGSystem.CategoryList[m_Page].Locations.Count-1, m_Gate));
			}
			...
			if( BID == 1 )
			{
				...
				if( Loc > PGC.Locations.Count ) return;
