	...
	public class Repair
	{
		...
		private class InternalTarget : Target
		{
			...
			private bool IsSpecialWeapon( BaseWeapon weapon )
			{
				...
				{
					if ( weapon is KilrathiHeavyGun || weapon is KilrathiGun || weapon is LightSword || weapon is DoubleLaserSword )
						return true;
				}
				else if ( m_CraftSystem is DefBlacksmithy )
				{
				...
				else if ( m_CraftSystem is DefTailoring ) // WIZARD ADDED
				{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( Equals( m_CraftSystem.CanCraft( from, m_Tool, targeted.GetType() ), 1044267 ) )
				{
