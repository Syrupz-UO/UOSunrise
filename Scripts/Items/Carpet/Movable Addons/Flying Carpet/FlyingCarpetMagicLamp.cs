...
namespace Server.Items
{
	...
	{
		protected override string _CantCreateMessage{ get{ return " "; } }
		...
		{
			if ( ( from.Map == Map.TerMur) && ( from.Y > 1800 || from.Y < 2864 ) ) { from.SendMessage( "The magic of the lamp does not seem to work here." ); return false; }
			else if ( ( from.Map == Map.Ilshenar) && ( from.X > 1620 ) ) { from.SendMessage( "The magic of the lamp does not seem to work here." ); return false; }
			else if ( ( from.Map == Map.Trammel) && ( from.X > 5120 || from.Y > 3127 ) ) { from.SendMessage( "The magic of the lamp does not seem to work here." ); return false; }
			else if ( ( from.Map == Map.Felucca) && ( from.X > 5120 || from.Y > 4096 ) ) { from.SendMessage( "The magic of the lamp does not seem to work here." ); return false; }
			else if ( ( from.Map == Map.Malas) && ( from.X > 1870 || from.Y > 2047 ) ) { from.SendMessage( "The magic of the lamp does not seem to work here." ); return false; }
			else if ( !IsChildOf( from.Backpack ) ) { from.SendMessage( "The magic lamp must be in your backpack to use." ); return false; }
			else { return true; }
		}
