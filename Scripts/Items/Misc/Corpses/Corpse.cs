...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
		...
		private static TimeSpan m_BoneDecayTime = TimeSpan.FromMinutes( 113.0 ); // WIZARD
		...
		public static Container Mobile_CreateCorpseHandler( Mobile owner, HairInfo hair, FacialHairInfo facialhair, List<Item> initialContent, List<Item> equipItems )
		{
			...
			if ( shouldFillCorpse )
			{
				...
				//if ( !owner.Player )        // WIZARD DOES NOT WANT INSTANCE LOOT
		...
		public virtual void Open( Mobile from, bool checkSelfLoot )
		{
			if ( from.AccessLevel > AccessLevel.Player || from.InRange( this.GetWorldLocation(), 2 ) )
			{
				...
				if ( selfLoot )
				{
					if ( from.QuestArrow != null ){ from.QuestArrow.Stop(); } // WIZARD - GET RID OF ARROW THAT POINTS TOWARD CORPSE OF PLAYER
