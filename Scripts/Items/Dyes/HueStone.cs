		...
		private class WHueTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( obj is Item )
				{
					...
					{
						if ( iDye is DDRelicRugAddonDeed ) // DJERYV ADDED TO DYE RELIC RUGS
						{
							DDRelicRugAddonDeed relic = (DDRelicRugAddonDeed)iDye;
							relic.RelicColor = m_Dye.Hue;
						}
						else if ( ( iDye is MyTentEastAddonDeed ) || ( iDye is MyTentSouthAddonDeed ) ) // DJERYV ADDED TO DYE TENTS
						{
