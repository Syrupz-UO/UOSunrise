...
namespace Server.Items
{
	...
	{
		[Constructable]
		public OilRuby() : this( 1 )
		{
		}

		[Constructable]
		public OilRuby( int amount ) : base( 0x1FDD )
		{
			Weight = 0.01;
			Stackable = true;
			Amount = amount;
		...
		private class OilTarget : Target
		{
			...
			public OilTarget( OilRuby tube ) : base( 1, false, TargetFlags.None )
			{
				m_Oil = tube;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( iOil is BaseWeapon )
				{
					...
					else if ( iOil.IsChildOf( from.Backpack ) && Server.Misc.MaterialInfo.IsMetalItem( iOil ) )
					{
						...
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Ruby", "IGNORED", MorphingTemplates.TemplateRuby("weapons") );
				...
				else if ( iOil is BaseArmor )
				{
					...
					{
						xOil.Resource = CraftResource.None;
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Ruby", "IGNORED", MorphingTemplates.TemplateRuby("armors") );
						from.RevealingAction();
						from.PlaySound( 0x23E );
						from.AddToBackpack( new Bottle() );
						m_Oil.Consume();
		...
		public OilRuby( Serial serial ) : base( serial )
		{
		}
