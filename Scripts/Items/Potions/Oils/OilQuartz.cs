...
namespace Server.Items
{
	public class OilQuartz : Item
	{
		...
		private class OilTarget : Target
		{
			...
			public OilTarget( OilQuartz tube ) : base( 1, false, TargetFlags.None )
			{
				m_Oil = tube;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( iOil is BaseWeapon )
				{
					...
					else if ( iOil.IsChildOf( from.Backpack ) && Server.Misc.MaterialInfo.IsMetalItem( iOil ) )
					{
						...
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Quartz", "IGNORED", MorphingTemplates.TemplateQuartz("weapons") );
				...
				else if ( iOil is BaseArmor )
				{
					...
					{
						xOil.Resource = CraftResource.None;
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Quartz", "IGNORED", MorphingTemplates.TemplateQuartz("armors") );
						from.RevealingAction();
						from.PlaySound( 0x23E );
						from.AddToBackpack( new Bottle() );
						m_Oil.Consume();
