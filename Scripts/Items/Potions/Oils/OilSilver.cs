...
namespace Server.Items
{
	...
	{
		[Constructable]
		public OilSilver() : this( 1 )
		{
		}

		[Constructable]
		public OilSilver( int amount ) : base( 0x1FDD )
		{
			Weight = 0.01;
			Stackable = true;
			Amount = amount;
		...
		private class OilTarget : Target
		{
			...
			public OilTarget( OilSilver tube ) : base( 1, false, TargetFlags.None )
			{
				m_Oil = tube;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( iOil is BaseWeapon )
				{
					...
					else if ( iOil.IsChildOf( from.Backpack ) && Server.Misc.MaterialInfo.IsMetalItem( iOil ) )
					{
						...
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Silver", "IGNORED", MorphingTemplates.TemplateSilver("weapons") );
				...
				else if ( iOil is BaseArmor )
				{
					...
					{
						xOil.Resource = CraftResource.None;
						MorphingItem.MorphMyItem( iOil, "IGNORED", "Silver", "IGNORED", MorphingTemplates.TemplateSilver("armors") );
						from.RevealingAction();
						from.PlaySound( 0x23E );
						from.AddToBackpack( new Bottle() );
						m_Oil.Consume();
		...
		public OilSilver( Serial serial ) : base( serial )
		{
		}
