...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		public virtual void BreathDamage_Callback( object state )
		{
			...
			if ( target == GetMaster() || (target is BaseCreature && ((BaseCreature)target).GetMaster() == GetMaster() || target is BaseCreature && ((BaseCreature)target).BreathImmune ))
				return;
		...
		public void DoFinalBreathAttack( Mobile target, int form, bool cycle )
		{
			...
			else if ( form == 2 ) // POTIONS THROWN -------------------------------------------------------------------------------------------------
			{
				...
				else if ( BreathEffectHue == 0xB92 )
				{
					...
					if ( !(Server.Items.HiddenTrap.SavingThrow( target, "Poison" )) )
					{
			...
			else if ( form == 5 ) // MANTICORE ------------------------------------------------------------------------------------------------------
			{
				...
				if ( target is PlayerMobile ){ Server.Misc.IntelligentAction.CryOut( target ); }
			}
		...
		*/
		...
		public virtual bool IsEnemy( Mobile m )
		{
			...
			if ( WhisperHue == 999 && Hidden && m is PlayerMobile ) // SURFACE FROM WATER AND ATTACK
			{
				...
				else // JUMP OUT OF WATER AND WALK TO SHORE
				{
					this.PlaySound( 0x026 );
					Effects.SendLocationEffect( this.Location, this.Map, 0x23B2, 16 );
				}
			...
			if ( reg.IsPartOf( typeof( NecromancerRegion ) ) && this.ControlSlots != 666 && GetPlayerInfo.EvilPlayer( m ) && (undead.Slays(this) || exorcism.Slays(this) || this is EvilMage) )
				return false;
		...
		public static bool AlwaysInvulnerable( Mobile m )
		{
			...
			else if ( m is BarFlyWizardWest ){ return true; }
			else if ( m is BarFlyWarriorEast ){ return true; }
			else if ( m is BarFlyWarriorNorth ){ return true; }
			else if ( m is BarFlyWarriorSouth ){ return true; }
			else if ( m is BarFlyWarriorWest ){ return true; }
			else if ( m is BarFlyWizardEast ){ return true; }
			else if ( m is BarFlyWizardNorth ){ return true; }
			else if ( m is BarFlyWizardSouth ){ return true; }
			else if ( m is Citizen ){ return true; }
		...
		{
			if ( m_dMinTameSkill <= 29.1 || m_bSummoned || m.AccessLevel >= AccessLevel.GameMaster || ( this is Alien && YellHue > 0 ) || ( this is Dragons && YellHue > 0 ) )
				return 1.0;
		...
		{
			// WIZARD DOES NOT WANT PARAGON BUNNIES
			// NOR DOES HE WANT PARAGONS IN THE BLACK KNIGHT CASTLE
...
// Xml Spawner 3.26c XmlSockets - SOF
			// WIZARD ADDED FOR MONSTER RED NAMES
			if ( m_FightMode == FightMode.Closest && !( this is BasePerson || this is BaseVendor || this is PlayerVendor ) )
			{
				this.NameHue = 0x22;
			}
		...
		public static void BeefUp( BaseCreature bc, int up )
		{
			...
			if ( Server.Misc.DifficultyLevel.CreaturesDetectHidden() )
			{
				double detectHidden = (double)(Server.Misc.DifficultyLevel.GetCreatureLevel( (Mobile)bc ) + 10);
		...
		{
			int Heat = DifficultyLevel.GetDifficultyLevel( this.Location, this.Map );
			...
			if ( Utility.RandomMinMax( 1, 4 ) == 1 && this.Map != Map.Ilshenar && Server.Misc.Worlds.IsMainRegion( Server.Misc.Worlds.GetRegionName( this.Map, this.Location ) ) ) // FOR HORSE RIDERS
			{
				if ( this is Berserker || this is Minstrel || this is Rogue || this is EvilMage || this is EvilMageLord || this is Brigand || this is Executioner || this is Monks )
				{
					new EvilHorse().Rider = this;
					ActiveSpeed = 0.1;
					PassiveSpeed = 0.2;
				}
				else if ( this is ElfBerserker || this is ElfMage || this is ElfRogue || this is ElfMinstrel || this is ElfMonks )
				{
					new EvilWolf().Rider = this;
					ActiveSpeed = 0.1;
					PassiveSpeed = 0.2;
				}
				else if ( this is OrkWarrior || this is OrkMage || this is OrkRogue || this is OrkMonks )
				{
					new EvilRidgeback().Rider = this;
					ActiveSpeed = 0.1;
					PassiveSpeed = 0.2;
				}
			}
			...
			if ( reg.IsPartOf( "the Ancient Crash Site" ) || reg.IsPartOf( "the Ancient Sky Ship" ) )
			{
				...
				else if ( this is Balron ){ this.Title = "the solar daemon"; this.Hue = 0xB08; this.BaseSoundID = 357; this.Body = 0x1AB; Name = NameList.RandomName( "demonic" ); this.AddItem( new LighterSource() ); }
			...
			if ( reg.IsPartOf( "Morgaelin's Inferno" ) )
			{
				if ( this is Daemon )
				{
					this.Name = NameList.RandomName( "demonic" );
					this.Title = "the daemon";
					this.Body = 9;
					this.BaseSoundID = 357;
					this.Hue = Utility.RandomList( 0x776, 0x86C, 0x701, 0xB12, 0xB38, 0xB13, 0x827, 0xAB3, 0xAFA, 0x93D, 0xB54, 0x775, 0x779 );
					AddItem( new LighterSource() );
				}
				else if ( this is Balron )
				{
					this.Name = NameList.RandomName( "demonic" );
					this.Title = "the balron";
					this.Body = 320;
					this.BaseSoundID = 357;
					this.Hue = Utility.RandomList( 0x776, 0x86C, 0x701, 0xB12, 0xB38, 0xB13, 0x827, 0xAB3, 0xAFA, 0x93D, 0xB54, 0x775, 0x779 );
					AddItem( new LighterSource() );
				...
				if ( this is Urk )
				{
					...
					if ( DressUpAs < 3 )
					{
						...
						switch ( Utility.Random( 28 ))
						{
							...
							case 14: weapon = new BoneHarvester(); weapon.Name = "bone harvester"; break;
			...
			if ( reg.IsPartOf( "the Caverns of Poseidon" ) )
			{
				...
				else if ( this is ForestDemon )
				{
					this.Title = "of the deep growth";
				}
			...
			{
				if ( this is LivingShadowIronStatue ){ this.Body = 189; }
				if ( this is LivingGoldStatue ){ this.Body = 189; }
				if ( this is LivingBronzeStatue ){ this.Body = 189; }
			}
		...
		public override void OnDamage( int amount, Mobile from, bool willKill )
		{
			...
			if ( this.WhisperHue == 999 && this.Z < -2 && from.Z >=0 && from is PlayerMobile && from.InRange( this.Location, 15 ) && from.Alive && from.Map == this.Map && CanSee( from ) )
			{
				this.Home = this.Location; // SO THEY KNOW WHERE TO GO BACK TO
			...
			if ( !Summoned && willKill )
				LevelItemManager.CheckItems( from, this ); // FOR GOD LEVEL ITEMS
		...
		{
			if ( WhisperHue == 999 && Hidden && CantWalk && !CanSwim )
			{
				this.PlaySound( 0x026 );
				Effects.SendLocationEffect( this.Location, this.Map, 0x23B2, 16 );
				this.CantWalk = false;
		...
		public virtual bool Teach( SkillName skill, Mobile m, int maxPointsToLearn, bool doTeach )
		{
			...
			switch ( res )
			{
				...
				case TeachResult.SkillNotRaisable:
				{
					...
					//m.SendLocalizedMessage( 501510, "", 0x22 ); // WIZARD HAS 1,000 SKILL POINT CAP
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( CanTeach && from.Alive && from.Kills < 1 && from.Criminal == false )
			{
		...
		public virtual bool CheckIdle()
		{
			...
			if ( Body.IsHuman && !Mounted ) // WIZARD ADDED SO NPCS DON'T DISMOUNT FROM MOUNTS WHEN IDLE
			{
				switch ( Utility.Random( 2 ) )
				{
		...
		{
			if ( Utility.RandomMinMax( 1 , 4 ) == 1 ){ PackItem( Loot.RandomNecromancyReagent() ); } // WIZARD ADDED FOR UNIDENTIFIED REAGENTS
			else
			{
				Item wizreg = Loot.RandomSecretReagent();
				UnknownReagent myreg = (UnknownReagent)wizreg;
		...
		public void PackReg( int amount )
		{
			...
			if ( Utility.RandomMinMax( 1 , 4 ) == 1 ){ PackItem( reg ); } // WIZARD ADDED FOR UNIDENTIFIED REAGENTS
			else
			{
				Item wizreg = Loot.RandomSecretReagent();
				UnknownReagent myreg = (UnknownReagent)wizreg;
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			{
				 int level;
				level = Server.Misc.DifficultyLevel.GetCreatureLevel(this);
			list.Add("Power Level: {0}",level);	
			}	
	...
	#region FS:ATS Edits
			if ( this.Tamable == true && FSATS.EnablePetBreeding == true )
			{
				...
				if ( nolevel != true )
				{
					...
					if ( this.Controlled == false )
						list.Add("Power Level: {0}",Server.Misc.DifficultyLevel.GetCreatureLevel(this));
			...
			if ( this is HenchmanFamiliar ) // WIZARD ADDED FOR FAMILIARS
				list.Add( "(familiar)" );
			else if ( this is PackBeast ) // WIZARD ADDED FOR PACK ANIMALS
				list.Add( "(Pack Animal)" );
			else if ( this is GolemPorter || this is GolemFighter ) // WIZARD ADDED FOR GOLEM
				list.Add( "(automaton)" );
			else if ( this is Robot ) // WIZARD ADDED FOR ROBOT
				list.Add( "(robot)" );
			else if ( this is FrankenPorter || this is FrankenFighter ) // WIZARD ADDED FOR FLESH GOLEM
				list.Add( "(reanimation)" );
			else if ( Summoned && !IsAnimatedDead && !IsNecroFamiliar )
				list.Add( 1049646 ); // (summoned)
			else if ( Controlled && Commandable && !(this is FrankenFighter) && !(this is FrankenPorter) && !(this is Robot) && !(this is GolemFighter) && !(this is GolemPorter) && !(this is PackBeast) && !(this is HenchmanMonster) && !(this is HenchmanFighter) && !(this is HenchmanWizard) && !(this is HenchmanArcher) && !(this is HenchmanFamiliar) )
			{
		...
		public override bool OnBeforeDeath()
		{
			...
			Mobile slayer = this.FindMostRecentDamager(true); // WIZARD ADDED FOR NPC KILLER EXPLOIT
			int Heat = DifficultyLevel.GetDifficultyLevel( this.Location, this.Map );
			if ( slayer is BasePerson )
			{
				this.Delete();
			}
			...
			if ( !Summoned && !NoKillAwards && !IsBonded && treasureLevel >= 0 )
			{
				...
				else if ( TreasureMap.LootChance >= Utility.RandomDouble() ) // WIZARD TWEAKED FOR TREASURE MAPS TO MATCH DUNGEON REGIONS
				{
...
/////////////////////////////////////////////////////////////////////////////////	
		...
		public override void OnDeath( Container c )
		{
			...
			else
			{
				if ( !Summoned && !m_NoKillAwards )
				{
					...
					for ( int i = 0; i < list.Count; ++i )
					{
						...
						/////////////////lootlist.cs in customs
			//	ArtifactValidate.ArtiChance(ds.m_Mobile, this); //Doc Loot System!
					////////////////////////////////////////////////
			...
			//	ArtifactValidate.MultiP( 1 ); //DOC LOOT SYSTEM!
				base.OnDeath( c );
...
public virtual bool HoldSmartSpawning
		...
		public virtual bool IsDispellable { get { return ( Summoned || DispelDifficulty > 0 ) && !IsAnimatedDead; } } // WIZARD ADD MORE OPTIONS
		...
		public virtual void OnThink()
		{
			...
			if ( this is TownGuards // GUARDS SHOULD MOVE BACK TO THEIR POST
				&& 
			...
			else if ( WhisperHue == 999 && CanSwim && !CantWalk && Combatant == null && !Hidden ) // DIVE UNDER WATER AND WAIT FOR VICTIM
			{
				bool dive = true;
			...
			foreach ( Mobile m in master.GetMobilesInRange( 10 ) )
			{
				if ( m is BaseCreature )
				{
					...
					if ( pet.Controlled && pet.ControlMaster == master )
					{
						...
						else if ( pet is HenchmanFamiliar || pet is PackBeast || pet is Robot || pet is GolemPorter || pet is GolemFighter || pet is FrankenPorter || pet is FrankenFighter ){ move.Add( pet ); }
					}
				}
			}
