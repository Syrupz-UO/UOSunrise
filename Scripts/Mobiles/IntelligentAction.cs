...
namespace Server.Misc
{
    class IntelligentAction
    {
		...
		public static void DoSpecialAbility( Mobile from, Mobile target )
		{
			...
			if ( Utility.RandomMinMax( 1, 20 ) == 1 && from.EmoteHue > 0 )
			{
				...
				foreach ( Mobile m in from.GetMobilesInRange( 10 ) )
				{
					...
					{
						if ( m is SnowElemental || m is IceSerpent || m is IceElemental || m is FrostOoze || m is FrostSpider || m is IceGolem || m is IceToad || m is IceSerpent )
							++monsters;
					}
		...
		public static void DressUpWizards( Mobile from )
		{
			...
			{ 
				Item robe = new Robe();
					...
					int wear = Utility.RandomMinMax( 0, 8 );
			...
			if ( Utility.RandomMinMax( 1, 4 ) > 1 )
			{
				...
				staff.Name = "staff";
				staff.ItemID = Utility.RandomList( 0xDF0, 0x13F8, 0xE89, 0x2D25, 0x26BC, 0x26C6, 0xDF2, 0xDF3, 0xDF4, 0xDF5, 0x269D, 0x269E );
				if ( staff.ItemID == 0x26BC || staff.ItemID == 0x26C6 ){ staff.Name = "scepter"; }
				if ( staff.ItemID == 0x269D || staff.ItemID == 0x269E ){ staff.Name = "skull scepter"; }
		...
		public static void DressUpFighters( Mobile m, string race )
		{
			m.AddItem( new Boots( Utility.RandomNeutralHue() ) );
			if ( 1 == Utility.RandomMinMax( 1, 2 ) ){ m.AddItem( new Cloak( RandomThings.GetRandomColor( 0 ) ) ); }
		...
		public static void DressUpRogues( Mobile m, string race )
		{
			BaseCreature bc = (BaseCreature)m;

			Region reg = Region.Find( bc.Home, bc.Map );

			int clothHue = Utility.RandomList( 0x973, 0x966, 0x96D, 0x972, 0x8A5, 0x979, 0x89F, 0x8AB, 0x66D, 0x8A8, 0x455, 0x851, 0x8FD, 0x8B0, 0x283, 0x227, 0x1C1, 0x8AC, 0x845, 0x851, 0x497, 0, Utility.RandomMetalHue(), Utility.RandomMetalHue(), Utility.RandomMetalHue(), Utility.RandomMetalHue(), Utility.RandomMetalHue(), Utility.RandomMetalHue(), Utility.RandomMetalHue() );
				...
				if ( killer is PlayerMobile )
				{
					...
					switch ( Utility.RandomMinMax( 0, 9 ) )
					{
						...
						case 4:	ScrollText = QuestCharacters.ParchmentWriter() + ",<br><br>I have another problem for you to take care of. A " + RandomThings.GetRandomJob() + " in " + RandomThings.GetRandomCity() + " is paying us " + gold.ToString() + ",000 gold to assassinate the one who killed their friend. I think we should send " + from.Name + " to deal with " + a + ", and " + b + " is known as " + killer.Name + ". I believe they headed in the direction of " + Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) + ". Have them wait for " + a + " there.<br><br> - " + QuestCharacters.ParchmentWriter();
							break;
						case 5:	ScrollText = from.Name + ",<br><br>I was in " + RandomThings.GetRandomCity() + " and I heard some whispers of a " + gold.ToString() + ",000 gold bounty on " + killer.Name + ". I also heard that " + RandomThings.GetRandomSociety() + " is the one offering the gold. I don't need to remind you that we have been trying to gain their trust so we can acquire " + Server.Misc.QuestCharacters.QuestItems() + ". I paid the barkeep a few coins and found out where " + b + " might be. I will check out the place nearby. You head to " + Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) + " and look for " + a + " there. We will meet each other in " + RandomThings.GetRandomCity() + " in a few days. This could be the chance we were waiting for.<br><br> - " + QuestCharacters.ParchmentWriter();
							break;
						case 6:	ScrollText = from.Name + ",<br><br>You are to slay the one they call " + killer.Name + ". They have been a little too curious of what lies within " + Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) + ", and we do not need " + a + " getting in our way to find " + Server.Misc.QuestCharacters.QuestItems() + ". If " + b + " finds it before we do, it could mean our very lives " + RandomThings.GetRandomSociety() + " will want. I will be going to " + RandomThings.GetRandomCity() + " to get some supplies, but I will return soon.<br><br> - " + QuestCharacters.ParchmentWriter();
							break;
						case 7:	ScrollText = from.Name + ",<br><br>Now that you eliminated " + QuestCharacters.ParchmentWriter() + ", it is time for your next target. This will be well worth your time as the fee being paid for this one is " + gold.ToString() + ",000 gold. They are known as " + killer.Name + ", and " + b + " has been talking about exploring " + Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) + ". If this is true, then " + QuestCharacters.ParchmentWriter() + " would have to hide " + Server.Misc.QuestCharacters.QuestItems() + " elsewhere. You can guess by the gold being offered, that they would rather not resort to such actions. Take care of " + a + " and we will split the gold in " + RandomThings.GetRandomCity() + ".";
							break;
						case 8:	ScrollText = QuestCharacters.ParchmentWriter() + ",<br><br>" + killer.Name + " has meddled in our plans for the last time. " + QuestCharacters.ParchmentWriter() + " claims that " + b + " doesn't even know of " + c + " participation in my annoyance, but the time to act is now. I think we should give this mission to " + from.Name + ", as they have yet to fail us. Send " + from.Name + " to " + Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) + " and kill " + a + " before " + b + " leaves the area. Bring back " + Server.Misc.QuestCharacters.QuestItems() + " if " + b + " is found with it.<br><br> - " + QuestCharacters.ParchmentWriter();
							break;
		...
		public static void ChooseMonk( Mobile m, string race )
		{
			...
			{
				PugilistGlove ring = new PugilistGlove();
		...
		public static void ChooseFighter( Mobile m, string race )
		{
			...
			{
				Server.Misc.IntelligentAction.DressUpFighters( m, race );
			}
			else 
			{
				Server.Misc.IntelligentAction.DressUpFighters( m, race );
		...
		public static void PunchStun( Mobile m )
		{
			if ( ( 100 - m.PhysicalResistance ) < Utility.RandomMinMax( 1, 100 ) && Utility.RandomMinMax( 1, 5 ) == 1 )
			{
				...
				m.Warmode = false;
				m.LocalOverheadMessage( MessageType.Emote, 0x22, true, "You are hit with a stunning punch!" );
			}
		}
		...
		public static void PoisonVictim( Mobile m, Mobile from )
		{
			...
			if ( !(Server.Items.HiddenTrap.SavingThrow( m, "Poison" )) && Utility.RandomMinMax( 1, 5 ) == 1 && CanPoison == true )
			{
				...
				m.LocalOverheadMessage(MessageType.Emote, 0x22, true, "You have been poisoned!");
			}
		}
			...
			if ( m != null && from != null && m.Hits > 0 && from is PlayerMobile && m.InRange( from, 2 ) && m.Map == from.Map )
			{
				if ( m.Skills[SkillName.Stealing].Value >= Utility.RandomMinMax( 1, 125 ) && m.Skills[SkillName.Snooping].Value >= Utility.RandomMinMax( 1, 100 ) )
				{
					if ( Utility.RandomMinMax( 1, 5 ) == 1 )
					{
						...
						{
							o++;
							if ( c == o ){ ((BaseCreature)m).PackItem( stuff ); from.LocalOverheadMessage(MessageType.Emote, 0x22, true, m.Name + " stole something from you!"); }
						}
					}
				}
			}
		...
		public static void CryOut( Mobile from )
		{
			if ( Utility.RandomMinMax( 1, 3 ) == 1 )
			{
				...
				{
					case 0: from.PlaySound( from.Female ? 0x31B : 0x42B ); break;
					case 1: from.PlaySound( from.Female ? 0x31C : 0x42C ); break;
					case 2: from.PlaySound( from.Female ? 0x32E : 0x440 ); break;
					case 3: from.PlaySound( from.Female ? 0x338 : 0x44A ); break;
					case 4: from.PlaySound( from.Female ? 0x324 : 0x434 ); break;
					case 5: from.PlaySound( from.Female ? 0x325 : 0x435 ); break;
					case 6: from.PlaySound( from.Female ? 0x326 : 0x436 ); break;
					case 7: from.PlaySound( from.Female ? 0x327 : 0x437 ); break;
					case 8: from.PlaySound( from.Female ? 0x328 : 0x438 ); break;
					case 9: from.PlaySound( from.Female ? 0x329 : 0x439 ); break;
					case 10: from.PlaySound( from.Female ? 0x32A : 0x43A ); break;
					case 11: from.PlaySound( from.Female ? 0x329 : 0x43B ); break;
					case 12: from.PlaySound( from.Female ? 0x32A : 0x43C ); break;
				}
			}
		}
		...
		public static void DropReagent( Mobile player, BaseCreature monster )
		{
			...
			if ( monster is Pixie && ( TestForReagent( player, "mixologist" ) ) )
			{
				switch ( Utility.RandomMinMax( 0, 2 ) ) 
				{
		...
		{
			if (	from is BoneMagi || from is SkeletalMage || from is SkeletalWizard || from is Lich || from is Vordo || from is LichLord || from is DemiLich || 
			...
			if ( from is Lich || from is LichLord || from is AncientLich || from is Surtaz || from is LichKing || from is DemiLich || from is UndeadDruid )
			{
				if ( killer != null )
				{
					string gear = "an old";
		...
		public static void BeforeMyBirth( Mobile from )
		{
			...
			else if ( from is BoneMagi || from is SkeletalMage || from is SkeletalWizard || from is Lich || from is Vordo || from is LichLord || from is DemiLich || from is AncientLich || from is Surtaz || from is LichKing )
			{
				from.EmoteHue = 13;
				PackGear = 0;
			}
			...
			else if ( from.EmoteHue == 7 )
			{
				...
				for ( int i = 0; i < from.Items.Count; ++i )
				{
					...
					if ( item is Hair || item is Beard )
						...
						item.Hue = Utility.RandomList( 0x47E, 0x47F, 0x481, 0x482 );
				}
			}
			else if ( from.EmoteHue == 8 )
			{
				...
				for ( int i = 0; i < from.Items.Count; ++i )
				{
					...
					if ( item is BaseClothing || item is BaseArmor )
						item.Hue = Utility.RandomList( 0x4E7, 0x4E7, 0x4E8, 0x4E9, 0x4EA, 0x4EB, 0x4EC );
				}
			}
			else if ( from.EmoteHue == 9 )
			{
		...
		{
			if ( m != null && from.EmoteHue != 505 && ((BaseCreature)from).ControlSlots != 666 && ((BaseCreature)from).GetMaster() == null && Utility.Random( 5 ) == 1 )
			{
				if ( from.SpeechHue < 1 ){ from.SpeechHue = Utility.RandomDyedHue(); }
				...
				if ( m is PlayerMobile )
				{
					...
					else if ( from is IceDevil || from is IceFiend )
					{
						switch ( Utility.Random( 4 ))		   
						{
					...
					else if ( from is ElfEvilSailorWizard
						 || from is ElfEvilSailorArcher
						 || from is ElfEvilSailorMinstrel
						 || from is EvilSailorArcher
						 || from is EvilSailorMinstrel
						 || from is EvilSailorWizard
						 || from is PirateCrewMage
						 || from is PirateLand
						 || from is ElfPirateCaptain
						 || from is ElfPirateCrew
						 || from is ElfPirateCrewBow
						 || from is ElfPirateCrewMage
						 || from is PirateCaptain
						 || from is PirateCrew
						 || from is PirateCrewBow )
					{
						switch ( Utility.Random( 9 ))		   
						{
				...
				if ( m is PlayerMobile )
				{
						 ...
						 || from is ArcticEttin )
					{
						switch ( Utility.Random( 2 ))		   
						{
							...
							case 2: from.Say("You hit me in the gonads!"); break;
						};
					}
					else if ( from is Titan
						 || from is ElderTitan
