...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
		...
		public virtual bool IsInvulnerable { get { return false; } } // WIZARD
			...
			public override void OnClick()
			{
				if ( m_Vendor.SupportsBulkOrders( m_From ) )
				{
					...
					else
					{
						...
						//if ( Core.SE ) // WIZARD DID THIS FOR LESS BULK ORDER TIMES
		...
		public static int BeggingPose( Mobile from ) // LET US SEE IF THEY ARE BEGGING - WIZARD
		{
			int beggar = 0;
		...
		public static int BeggingKarma( Mobile from ) // LET US SEE IF THEY ARE BEGGING - WIZARD
		{
			int charisma = 0;
		...
		public static string BeggingWords() // LET US SEE IF THEY ARE BEGGING - WIZARD
		{
		...
		public virtual TimeSpan RestockDelay
		{
			get
			{
				return TimeSpan.FromHours( 2 ); // WIZARD
			}
		}

		public virtual TimeSpan RestockDelayFull // WIZARD
		{
			get
			{
		...
		public virtual void InitBody()
		{
			...
			if ( this is Roscoe || this is Garth ) // WIZARD ADDED FOR SPECIAL VENDORS
			{
				Body = 0x190;
		...
		public override void OnAfterSpawn()
		{
			...
			else if ( reg.Name == "the Ship's Lower Deck" && this is Provisioner )
			{
		...
		public override bool OnBeforeDeath() // WIZARD MURDER SETTINGS
		{
			...
			return true;
		} // WIZARD END MURDER SETTINGS

		public override bool IsEnemy( Mobile m )
		{
			if ( DifficultyLevel.GetMyEnemies( m, this, true ) == false )
				return false;
		...
		private static TimeSpan InventoryDecayTime = TimeSpan.FromHours( 2.0 ); // WIZARD
				...
				foreach ( IShopSellInfo ssi in info )
				{
					...
					foreach ( Item item in items )
					{
						...
						if ( item.IsStandardLoot() && item.Movable && ssi.IsSellable( item ) )
						{
							...
							int barter = (int)from.Skills[SkillName.ItemID].Value; // WIZARD ADDED THE barter INT TO CHECK ITEMID SKILL
        ...
        public virtual bool OnSellItems( Mobile seller, List<SellItemResponse> list )
		{
			...
			foreach ( SellItemResponse resp in list )
			{
				...
				if ( BeggingPose(seller) > 0 ) // LET US SEE IF THEY ARE BEGGING - WIZARD
				{
		...
		public override void GetProperties( ObjectPropertyList list ) // WIZARD ADDED SO PLAYERS KNOW WHO BELONGS TO WHAT GUILD
		{
