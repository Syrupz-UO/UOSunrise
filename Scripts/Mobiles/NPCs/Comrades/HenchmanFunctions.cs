...
namespace Server.Misc
{
    class HenchmanFunctions
    {
		...
		{
			int nHealing = (int)m.Skills[SkillName.MagicResist].Value;

			if ( (m.Hunger > 0) && (m.Hits < m.HitsMax) && (nHealing >= Utility.Random( 100 )) )
			{
				if ( m.Poisoned ){ if ( nHealing>=50 ){ m.CurePoison( m ); } }
				else { m.Hits = m.HitsMax; }
				m.Hunger = m.Hunger - 1;
				m.RevealingAction();
				m.PlaySound( 0x57 );
			}
