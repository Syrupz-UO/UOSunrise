	...
	class SpiritSpeak
	{
		...
		private class SpiritSpeakSpell : Spell
		{
			...
			public override void OnCast()
			{
				...
				if ( toChannel != null )
				{
					min = 1 + (int)(Caster.Skills[SkillName.SpiritSpeak].Value * 0.25) + (int)(Caster.Skills[SkillName.Wrestling].Value * 0.15);
					max = min * 2; // WIZARD CHANGED TO MAKE MORE HEALTH
					mana = min;
					message = "You channel the corpse's energy to restore yourself.";
				}
				else
				{
					min = 1 + (int)(Caster.Skills[SkillName.SpiritSpeak].Value * 0.15) + (int)(Caster.Skills[SkillName.Wrestling].Value * 0.15);
					max = min * 2; // WIZARD CHANGED TO MAKE MORE HEALTH
					mana = max;
