...
namespace Server.Regions
{
	...
	public class BaseRegion : Region
	{
		...
		{
			// WIZARD SPELLS ONLY WORK 33% OF THE TIME IN THE ABYSS, UNLESS THEY CARRY THE ORB OR THE CODEX
			// ALL OTHER TYPES OF MAGIC WORK JUST FINE
			if ( Worlds.GetMyWorld( m.Map, m.Location, m.X, m.Y ) == "the Underworld" && s is MagerySpell && Utility.RandomMinMax( 1, 3 ) != 1 )
			{
				Item relic = m.FindItemOnLayer( Layer.Talisman );

				bool fizzle = true;

				if ( relic is OrbOfTheAbyss ){ fizzle = false; }
				else if ( relic is CodexWisdom ){ fizzle = false; }
