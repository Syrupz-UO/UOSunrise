...
namespace Server.Mobiles
{
	...
	public class AUnicorn : BaseMount
	{
		...
		{
			m.SendLocalizedMessage( 1042318 ); // The AUnicorn refuses to allow you to ride it.
		}
				...
				if( p != null )
				{
					...
					if( chanceToCure > Utility.Random( 100 ) )
					{
						if( Rider.CurePoison( this ) )	//TODO: Confirm if mount is the one flagged for curing it or the rider is
						{
							...
							Rider.PlaySound( 0xA9 );		// AUnicorn's whinny.
		...
		[Constructable]
		public AUnicorn() : this( "a Unicorn" )
		{
		}

		[Constructable]
		public AUnicorn( string name ) : base( name, 0x7A, 0x3EB4, AIType.AI_Mage, FightMode.Aggressor, 10, 1, 0.2, 0.4 )
		{
			BaseSoundID = 0x4BC;
		...
		public AUnicorn( Serial serial ) : base( serial )
		{
		}
