...
[CommandProperty(AccessLevel.GameMaster)]
		...
		public ChampionSpawn() : base( 0xBD2 )
		{
            ...
            m_ExpireDelay = TimeSpan.FromDays(365);
		...
		public void EndRestart()
		{
			if( RandomizeType )
			{
				switch( Utility.Random( 7 ) )
				{
                    ...
                    case 6: Type = ChampionSpawnType.Crafter; break;
					case 11:Type = ChampionSpawnType.WinterSolstice; break;
					case 12:Type = ChampionSpawnType.Brimstone; break;	 
				}
			}
		...
		public void OnSlice()
		{
			...
			if( m_Champion != null )
			{
				if( m_Champion.Deleted )
				{
					...
					if( m_Altar != null )
					{
						...
						if( !Core.ML || Map == Map.Felucca || Map == Map.TerMur || Map == Map.Malas || Map == Map.Trammel || Map == Map.Ilshenar || Map == Map.Tokuno || Map == Map.TerMur )
						{
			...
			{
		        	((Corpse)m.Corpse).BeginDecay( TimeSpan.FromSeconds( 5 ));
			}
						...
						if( killer is PlayerMobile )
						{
							...
							if ( Core.ML )
							{
                               if (Map == Map.Felucca || Map == Map.TerMur || Map == Map.Malas || Map == Map.Trammel || Map == Map.Ilshenar || Map == Map.Tokuno)
								{
									if ( Utility.RandomDouble() < 0.001 )
									{
										...
										if ( random <= 24 )
										{
											ScrollofTranscendence SoTF = CreateRandomSoT( );
        ...
        public void SpawnBlock()
        {
           ...
           int newBlock = Utility.RandomMinMax(5, 20);
               ...
               for (int i = 0; i < newBlock; ++i)
               {
                  ...
                  {
                      default:
                      case 1: case 2: case 3: case 4: Block = new Blood(); break;
                      case 5: case 6: case 7: case 8: Block = new BlackCrystal(); break; 
                      case 9: case 10: case 11: case 12: Block = new ChampField(); break;
                      case 13: case 14: case 15: case 16: Block = new ChampField2(); break;

                  }   

		  bool validLocation = false;
		  Point3D loc = m_Altar.Location;
       
		  for ( int j = 0; !validLocation && j < 10; ++j )
                  {
			int x = X + Utility.RandomMinMax(-30, 30);
			int y = Y + Utility.RandomMinMax(-30, 30);
        ...
        public void RemoveBlock()
        {
           ...
           IPooledEnumerable eable = map.GetItemsInRange( m_Altar.Location, 45 );

	   foreach( Item item in eable )
	   {

               if ( item is BlackCrystal || item is Blood ) 
                 list.Add(item); 
           } 
        ...
        public void SpawnTraps()
        {
           ...
           int newTraps = Utility.RandomMinMax(4, 10);
               ...
               for (int i = 0; i < newTraps; ++i)
               {
                  ...
                  {
                      default:
                      case 0: trap = new FireColumnTrap(); break;
                      case 1: trap = new GasTrap( Utility.RandomBool() ? GasTrapType.NorthWall : GasTrapType.WestWall ); break;
                      case 2: trap = new GiantSpikeTrap(); break;
                      case 3: trap = new SawTrap( Utility.RandomBool() ? SawTrapType.WestFloor : SawTrapType.NorthFloor ); break;
                      case 4: trap = new SpikeTrap( Utility.RandomBool() ? SpikeTrapType.WestFloor : SpikeTrapType.NorthFloor ); break;
                  }                        
		  ...
		  for ( int j = 0; !validLocation && j < 10; ++j )
		  {
		        int x = X + Utility.RandomMinMax(K, W);
			int y = Y + Utility.RandomMinMax(K, W);
			int z = map.GetAverageZ( x, y );
        ...
        public void RemoveTraps()
        {
           ...
           IPooledEnumerable eable2 = map.GetItemsInRange( m_Altar.Location, Q );

	   foreach( Item item2 in eable2 )
	   {

               if ( item2 is BaseTrap ) 
                 list2.Add(item2); 
           } 
        ...
        public void SpawnChampion()
        {
			...
			{
				// Modified for Daat99
				if ( m_Type == ChampionSpawnType.Crafter )   // to add parameter 'true' for Master of The Arts
					m_Champion = new MasterOfTheArts( true ) as Mobile;
				else
